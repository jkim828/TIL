...
정규분포는 여러 분포 중에서 가장 중요한 분포라고 할 수 있다. 정규분포는 실제로 많이 사용되기 때문에 중요하기도 하지만 이론적으로 중심극한정리가 성립하여 그 중요성이 부각된다고 할 수 있다. 중심극한정리란 간단히 말해 동일분포를 가지는 분포들의 평균은 그 개수가 많아지면서 언제나 정규분포로 수렴한다는 것이다. (네이버 지식백과)
